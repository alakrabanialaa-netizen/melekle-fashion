...
<main class="bg-[#fdfdfd] min-h-screen">
    ...
    <div class="container mx-auto px-4 pt-16 pb-12 text-center">
        <h1 class="text-4xl md:text-5xl font-black text-gray-900 mb-3 animate-fade-in">قسم البيبي الحديث</h1>
        <div class="w-20 h-1 bg-emerald-500 mx-auto mb-4 rounded-full"></div>
        <p class="text-gray-500 text-lg">أرقى وأنعم الأقمشة والتشكيلات المريحة لطفلك الصغير</p>
    </div>
    ...
    <div class="container mx-auto px-4 pb-24">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-8">
                @foreach($products as $product)
                    <div class="product-card-ty group relative flex flex-col justify-between">
                        ...
                        <a href="{{ route('products.show', [$product->id, $product->product_slug ?? 'baby-style']) }}" class="ty-image-wrapper block relative overflow-hidden rounded-t-xl">
                            ...
                            <div class="absolute inset-x-0 bottom-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300 bg-gradient-to-t from-black/60 to-transparent hidden md:block z-20">
                                <form action="{{ url('cart-add/'.$product->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="w-full bg-white text-gray-900 font-bold py-2 rounded-lg text-sm hover:bg-emerald-500 hover:text-white transition-colors">
                                        أضف إلى السلة
                                    </button>
                                </form>
                            </div>
                        ...
                        <div class="ty-info-wrapper flex flex-col justify-between p-4 flex-grow text-right">
                            <a href="{{ route('products.show', [$product->id, $product->product_slug ?? 'baby-style']) }}">
                                <h3 class="ty-title mb-2 hover:text-emerald-600 transition-colors">{{ $product->name }}</h3>
                            </a>
                            ...
                            <div class="flex items-center justify-between mt-auto pt-2">
                                <div class="flex flex-col">
                                    ...
                                    @endif
                                    <span class="ty-final-price text-emerald-600 font-black text-lg">{{ number_format($product->price, 2) }} ₺</span>
                                </div>
                                
                                <form action="{{ url('cart-add/'.$product->id) }}" method="POST" class="md:hidden">
                                    @csrf
                                    <button type="submit" class="w-10 h-10 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-all">
        ...
        @else
            ...
            <div class="text-center py-32 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
                ...
                <p class="text-gray-500 mb-8">نحن نجهز تشكيلة حديثي الولادة والمواليد، انتظرونا!</p>
                <a href="{{ url('/') }}" class="inline-block px-10 py-3 bg-gray-900 text-white font-bold rounded-full hover:bg-emerald-500 transition-all">
                    العودة للرئيسية
                </a>
            </div>
        @endif
            ...
            <div class="grid md:grid-cols-4 gap-12 mb-16">
                <div>
                    ...
                    <div class="flex gap-4 mt-6 text-xl">
                        <a href="https://www.instagram.com/meleklerkids/" target="_blank" rel="noopener noreferrer" class="hover:text-emerald-500 transition"><i class="fab fa-instagram"></i></a>
                        <a href="https://www.facebook.com/MELEKLERKIDSTR" target="_blank" rel="noopener noreferrer" class="hover:text-emerald-500 transition"><i class="fab fa-facebook"></i></a>
                        <a href="https://api.whatsapp.com/message/CL67ADRC7PMFO1" target="_blank" rel="noopener noreferrer" class="hover:text-emerald-500 transition"><i class="fab fa-whatsapp"></i></a>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex">
                        ...
                        <button class="px-5 bg-emerald-500 text-white rounded-l-xl hover:bg-emerald-600 transition font-bold text-sm">اشتراك</button>
                    </div>
                </div>
            </div>
