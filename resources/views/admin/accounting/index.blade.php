...
<div class="container mx-auto p-6 space-y-8 bg-gray-50 min-h-screen" dir="rtl">
    ...
    {{-- ================= 💰 قسم 1: المؤشرات المادية الدقيقة (الخانات الأربعة الأساسية) ================= --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {{-- خانة صافي المربح والخسارة الديناميكية --}}
        <div class="{{ $netProfit >= 0 ? 'bg-indigo-50 border-indigo-200' : 'bg-red-50 border-red-200' }} p-6 rounded-2xl border shadow-xs">
            <span class="text-xs font-bold {{ $netProfit >= 0 ? 'text-indigo-700' : 'text-red-700' }} block uppercase">صافي المربح / الخسارة</span>
            <p class="text-3xl font-black {{ $netProfit >= 0 ? 'text-indigo-700' : 'text-red-700' }} mt-2 font-mono">
                {{ $netProfit >= 0 ? '+' : '' }}{{ number_format($netProfit ?? 0, 2) }} ₺
            </p>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        <div class="flex justify-between items-center border-b pb-4 mb-4">
            <div>
                <h2 class="text-xl font-bold text-gray-800">تفاصيل وجرد بضاعة المستودع الحالية</h2>
                <p class="text-xs text-gray-400">قائمة تفصيلية بكل المنتجات المتوفرة في المخازن</p>
            </div>
            <span class="text-xs bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-lg font-bold">إجمالي قطع المستودع: {{ number_format($totalStockPieces) }} قطعة</span>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            <table class="w-full text-right text-xs">
                <tbody>
                    ...
                    <tr class="border-b"><td class="py-2.5">{{ $exp->description }}</td><td class="py-2.5 font-bold text-rose-600 font-mono">{{ number_format($exp->amount, 2) }} ₺</td></tr>
                    @endforeach
                </tbody>
            </table>
        </div>

        {{-- حركات رأس المال والتمويل --}}
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            <table class="w-full text-right text-xs">
                <tbody>
                    ...
                    <tr class="border-b"><td class="py-2.5">{{ $cap->description }}</td><td class="py-2.5 font-bold text-emerald-600 font-mono">{{ number_format($cap->amount, 2) }} ₺</td></tr>
                    @endforeach
                </tbody>
            </table>
        </div>
